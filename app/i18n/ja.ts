...
export const ja: typeof en = {
  ...
  structuredDataGeneratorPage: {
    ...
    lead: '構造化データはSEO対策に必須だ、みたいに言われるけど、どうやって書いたらいいのか、どこに貼り付けたらいいのかわからない。そんな経験はありませんか？\n構造化データは、「誰が書いた記事か」「どこにある店か」を検索エンジンやAIが読める形（JSON-LD）でページの中に書き込んでおく情報です。検索結果に評価や価格が出るリッチリザルトの条件になり、最近ではAIの回答に引用されやすくするAEOにも効くと言われています。\nよしにゃに構造化データ作成なら、書き方が分からなくても大丈夫。種類を選んで分かる項目だけ埋めれば、正しいJSON-LDがその場で組み上がり、どのページのどこに貼るかも案内します。あとはコードをサイトに貼るだけ。\n入力内容はサーバーに送られず、ブラウザ内だけで完結するので、会社の住所や電話番号を入れても安心です。',
    privacyNote:
