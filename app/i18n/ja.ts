...
export const ja: typeof en = {
  ...
  csvEncodingFixerGuide: {
    ...
    sections: [
      {
        heading: 'スプレッドシートでは読めるのに、Excelで文字化けする理由',
        body: 'CSVの中には「この文字コードで書かれている」という情報が記録されていないため、どのソフトも自分で判断するしかありません。Googleスプレッドシートは中身を調べ、既定をUTF-8として扱います。一方Excelは、CSVをダブルクリックで開くとき中身を調べず、OSの既定コードページ——日本語Windowsでは Shift_JIS——で読みます。UTF-8の日本語は1文字3バイトですが、Shift_JISは2バイト単位で読むため、区切り位置がずれて意味不明な文字の羅列になります。ファイル自体は壊れていません。同じファイルでも、「データ」→「テキストまたはCSVから」から読み込めば中身を判定するため、Excelでも正しく開けます。',
      },
      {
        heading: 'このページで使う言葉',
        body: 'このツールが扱うのは、次の3つの言葉だけです。',
        terms: [
          {
            term: 'UTF-8',
            definition:
              '現在ほとんどの場面で使われている文字コードです。世界中の言語を同時に扱えるため、標準として定着しました。',
          },
          {
            term: 'Shift_JIS',
            definition:
              '古くからある日本語の文字コードです。業務システムからの出力や、ExcelがCSVを保存するときに今も使われます。最近のソフトはこれを前提にしないため、行き違いが起きます。',
          },
          {
            term: 'BOM',
            definition:
              'ファイルの先頭に付く3バイトの目印で、「これはUTF-8です」と示すものです。Excelはこれが無いと信用しません。エディタ上では見えず、データの中身は変わりません。',
          },
        ],
      },
      {
        heading: 'このツールにできないこと',
        body: '間違った文字コードで読み込まれた状態で保存されてしまったファイルは、元の文字が失われているため、どんなツールでも復元できません。出力し直してもらう必要があります。このツールはその状態を検出したときに、その旨をお伝えします。見た目だけ整えて、同じ欠落を抱えたファイルをお返しすることはしません。また、Shift_JISへの変換には対応していません。Shift_JISしか受け付けないシステムに提出する用途には使えません。',
      },
      {
        heading: '使い方',
        steps: [
          'アップロードエリアにCSVをドロップするか、クリックして選択します。ブラウザ内で読み込まれ、アップロードは行われません。',
          'ファイルごとの判定結果を確認します。実際の文字コードと、変換が必要なのか目印を付けるだけで済むのかが表示されます。',
          'プレビューを開いて、文字が正しく読めているか確認します。プレビューが正しければ、修復結果も正しくなります。',
          '「修復したファイルをダウンロード」を押します。UTF-8かつExcelが必要とする目印付きのコピーが、元のファイルとは別に保存されます。',
        ],
      },
      {
        heading: 'こんなときに便利',
        items: [
          '会計ソフトやECサイトから出力したCSVが、Excelでは意味不明な文字なのに、スプレッドシートでは普通に読める。',
          '同僚から届いたCSVが、相手の環境では正しく開けるのに自分の環境では化ける。',
          '社内の古いシステムからの出力が今もShift_JISで、取り込み先はUTF-8を求めている。',
          '取引先に送る前に、そのファイルの文字コードを確認しておきたい。',
          'UTF-8しか受け付けないシステムに取り込む予定があり、エラーで気づく前に確かめておきたい。',
        ],
      },
      {
        heading: 'プライバシーと安全性',
        body: 'ファイルの読み込み・判定・変換は、すべてブラウザの中だけで行われます。サーバーへのアップロードはなく、アカウント登録も不要で、ページを閉じるか再読み込みすればデータは消えます。ファイルが端末から出ないため、顧客名簿などの機密性が高い出力データに対しても、オンラインサービスへのアップロードが禁止されている職場で使えます。',
      },
    ],
    faqHeading: 'よくある質問',
    faq: [
      {
        ...
        answer:
          'いいえ。ファイルの読み込みも書き換えも、お使いの端末のメモリ上でブラウザが行います。中身・ファイル名のいずれも送信されません。',
      },
      {
        question: 'データが変わってしまいませんか？',
        answer:
          'すでにUTF-8のファイルは、バイト列がそのまま保持され、先頭に3バイトの目印が加わるだけです。他の文字コードのファイルは変換されるため、バイト列は変わりますが、文字そのものは変わりません。いずれの場合も、ディスク上の元ファイルはそのまま残り、別のコピーがダウンロードされます。',
      },
      {
        question: 'ダウンロードしたファイルの名前が変わっているのはなぜですか？',
        answer:
          '修復しても、行も列も見た目は同じで、外から区別が付きません。_utf8 という接尾辞は、ダウンロードフォルダに並んだときに元のファイルと取り違えないためのものです。',
      },
      {
        question: 'Excelファイル（.xlsx）が文字化けしています。直せますか？',
        answer:
          'いいえ。そもそも .xlsx はこの問題が起きません。中身の文字は常にUTF-8で保存される仕組みだからです。.xlsx が文字化けしている場合、保存された時点ですでに壊れているため、元のデータから作り直す必要があります。',
      },
      {
        question: 'プレビューを見ても、まだ意味不明な文字のままです。',
        answer:
          'その場合、ファイルが手元に届く前に壊れている可能性が高いです。間違った文字コードで読み込まれた状態で保存されたものと思われます。出力元に再出力を依頼し、可能であればUTF-8での出力をご相談ください。',
      },
      {
        question: 'Shift_JISに変換することはできますか？',
        answer:
          '現在は対応していません。ブラウザは古い日本語の文字コードを読むことはできますが、書き出せるのはUTF-8だけという制約があるためです。Excelで開くことが目的であれば、このツールが付ける目印だけで解決します。',
      },
      {
        question: '一度に何件まで処理できますか？',
        answer:
          '50件、1ファイル50MBまでです。表計算として解析するのではなくバイト列を読むだけの処理なので、まとめて読み込んでもほぼ一瞬で終わります。',
      },
    ],
  ...
  csvEncodingFixerPage: {
    ...
    metaDescription:
      'スプレッドシートでは正常なのにExcelで開くと文字化けするCSVは、壊れているわけではありません。Excelが文字コードを取り違えているだけです。実際の文字コードを判定し、Excelで正しく開けるファイルとして保存します。ファイルはサーバーに送信されません。',
    heading: 'よしにゃにCSV文字化け修復',
    toolName: 'よしにゃにCSV文字化け修復',
    toolDescription:
      'CSVの本当の文字コードを判定し、Excelで正しく開けるファイルとして保存します。',
    lead: 'ExcelでCSVを開いたら意味不明な文字が並んでいたのに、同じファイルをスプレッドシートで開くと普通に読める。そんなことはありませんか？それは実はファイルが壊れているのではなく、Excelが文字コードを間違えて読んでいるだけです。このツールが本当の文字コードを判定し、Excelで正しく開けるファイルとして保存します。',
    privacyNote:
      'ファイルはサーバーに送信されません。すべての処理はブラウザ内で完結します。',
  },
  csvEncodingFixer: {
    ...
    verdictBomOnly:
      'このファイルの中身はUTF-8で、壊れていません。Excelがそう読むために必要な目印が、先頭に付いていないだけです。ダウンロードすると、その3バイトを追加します。中身は1バイトも変わりません。',
    verdictConvert: (from: string) =>
      `このファイルの中身は${from}です。Excelをはじめ最近のソフトはこの文字コードを前提にしないため、文字化けします。ダウンロードするとUTF-8に変換し、Excelで正しく開けるようにします。`,
    verdictAlreadyFine:
      'このファイルはUTF-8で、Excelが必要とする目印も付いています。そのままで正しく開けるはずです。',
    damagedWarning:
