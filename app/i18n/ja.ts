...
export const ja: typeof en = {
  ...
  structuredDataGuide: {
    ...
    sections: [
      ...
      {
        heading: '構造化データとは',
        body: '構造化データは、そのページが何についてのページかを、検索エンジン共通の語彙（schema.org）で書いたJSON-LDのブロックです。「この記事はこの著者がこの日に書いた」「この店はこの住所で、この時間に開いている」といった内容で、訪問者には見えません。検索エンジンはこれを使ってページを理解し、種類によっては評価・価格・パンくずなどを添えた、より目立つ検索結果（リッチリザルト）を表示します。それ自体で順位が上がるものではなく、結果を分かりやすくしてクリックされやすくするためのものです。',
      },
      {
        heading: 'どの種類を使うか',
        terms: [
  ...
  structuredDataPage: {
    ...
    lead: 'リッチリザルトを出したくて schema.org のドキュメントを開き、どこかのサンプルをコピーして値を書き換え、閉じ忘れた括弧や余計な項目で検証ツールに突き返される——構造化データの作成で手間取ったことはありませんか？しかも通ったあとで「これ、どのページに貼るの？」と迷います。\nよしにゃに構造化データ作成なら、種類を選んでフォームを埋めるだけ。入力するそばからJSON-LDが組み上がり、埋めた項目だけが出力され、入れ子の @type も自動で付きます。どのページのどこに貼るかも種類ごとに案内します。\n入力内容はサーバーに送られず、ブラウザ内だけで完結するので、会社の住所や電話番号を入れても安心です。',
    privacyNote:
